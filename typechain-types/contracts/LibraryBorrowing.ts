/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import type {
  BaseContract,
  BigNumber,
  BigNumberish,
  BytesLike,
  CallOverrides,
  ContractTransaction,
  Overrides,
  PopulatedTransaction,
  Signer,
  utils,
} from "ethers";
import type {
  FunctionFragment,
  Result,
  EventFragment,
} from "@ethersproject/abi";
import type { Listener, Provider } from "@ethersproject/providers";
import type {
  TypedEventFilter,
  TypedEvent,
  TypedListener,
  OnEvent,
  PromiseOrValue,
} from "../common";

export declare namespace LibraryFactory {
  export type BookStruct = {
    id: PromiseOrValue<BigNumberish>;
    isbn: PromiseOrValue<string>;
    title: PromiseOrValue<string>;
    author: PromiseOrValue<string>;
    borrowed: PromiseOrValue<boolean>;
    borrowingAddress: PromiseOrValue<string>;
  };

  export type BookStructOutput = [
    BigNumber,
    string,
    string,
    string,
    boolean,
    string
  ] & {
    id: BigNumber;
    isbn: string;
    title: string;
    author: string;
    borrowed: boolean;
    borrowingAddress: string;
  };
}

export interface LibraryBorrowingInterface extends utils.Interface {
  functions: {
    "addBook(string,string,string)": FunctionFragment;
    "books(uint256)": FunctionFragment;
    "borrowBook(string)": FunctionFragment;
    "copies(string)": FunctionFragment;
    "getBooks()": FunctionFragment;
    "historicalBorrowers(string,uint256)": FunctionFragment;
    "owner()": FunctionFragment;
    "renounceOwnership()": FunctionFragment;
    "returnBook(string)": FunctionFragment;
    "showAvailableBooks()": FunctionFragment;
    "titleToIsbn(string)": FunctionFragment;
    "transferOwnership(address)": FunctionFragment;
  };

  getFunction(
    nameOrSignatureOrTopic:
      | "addBook"
      | "books"
      | "borrowBook"
      | "copies"
      | "getBooks"
      | "historicalBorrowers"
      | "owner"
      | "renounceOwnership"
      | "returnBook"
      | "showAvailableBooks"
      | "titleToIsbn"
      | "transferOwnership"
  ): FunctionFragment;

  encodeFunctionData(
    functionFragment: "addBook",
    values: [
      PromiseOrValue<string>,
      PromiseOrValue<string>,
      PromiseOrValue<string>
    ]
  ): string;
  encodeFunctionData(
    functionFragment: "books",
    values: [PromiseOrValue<BigNumberish>]
  ): string;
  encodeFunctionData(
    functionFragment: "borrowBook",
    values: [PromiseOrValue<string>]
  ): string;
  encodeFunctionData(
    functionFragment: "copies",
    values: [PromiseOrValue<string>]
  ): string;
  encodeFunctionData(functionFragment: "getBooks", values?: undefined): string;
  encodeFunctionData(
    functionFragment: "historicalBorrowers",
    values: [PromiseOrValue<string>, PromiseOrValue<BigNumberish>]
  ): string;
  encodeFunctionData(functionFragment: "owner", values?: undefined): string;
  encodeFunctionData(
    functionFragment: "renounceOwnership",
    values?: undefined
  ): string;
  encodeFunctionData(
    functionFragment: "returnBook",
    values: [PromiseOrValue<string>]
  ): string;
  encodeFunctionData(
    functionFragment: "showAvailableBooks",
    values?: undefined
  ): string;
  encodeFunctionData(
    functionFragment: "titleToIsbn",
    values: [PromiseOrValue<string>]
  ): string;
  encodeFunctionData(
    functionFragment: "transferOwnership",
    values: [PromiseOrValue<string>]
  ): string;

  decodeFunctionResult(functionFragment: "addBook", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "books", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "borrowBook", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "copies", data: BytesLike): Result;
  decodeFunctionResult(functionFragment: "getBooks", data: BytesLike): Result;
  decodeFunctionResult(
    functionFragment: "historicalBorrowers",
    data: BytesLike
  ): Result;
  decodeFunctionResult(functionFragment: "owner", data: BytesLike): Result;
  decodeFunctionResult(
    functionFragment: "renounceOwnership",
    data: BytesLike
  ): Result;
  decodeFunctionResult(functionFragment: "returnBook", data: BytesLike): Result;
  decodeFunctionResult(
    functionFragment: "showAvailableBooks",
    data: BytesLike
  ): Result;
  decodeFunctionResult(
    functionFragment: "titleToIsbn",
    data: BytesLike
  ): Result;
  decodeFunctionResult(
    functionFragment: "transferOwnership",
    data: BytesLike
  ): Result;

  events: {
    "OwnershipTransferred(address,address)": EventFragment;
  };

  getEvent(nameOrSignatureOrTopic: "OwnershipTransferred"): EventFragment;
}

export interface OwnershipTransferredEventObject {
  previousOwner: string;
  newOwner: string;
}
export type OwnershipTransferredEvent = TypedEvent<
  [string, string],
  OwnershipTransferredEventObject
>;

export type OwnershipTransferredEventFilter =
  TypedEventFilter<OwnershipTransferredEvent>;

export interface LibraryBorrowing extends BaseContract {
  connect(signerOrProvider: Signer | Provider | string): this;
  attach(addressOrName: string): this;
  deployed(): Promise<this>;

  interface: LibraryBorrowingInterface;

  queryFilter<TEvent extends TypedEvent>(
    event: TypedEventFilter<TEvent>,
    fromBlockOrBlockhash?: string | number | undefined,
    toBlock?: string | number | undefined
  ): Promise<Array<TEvent>>;

  listeners<TEvent extends TypedEvent>(
    eventFilter?: TypedEventFilter<TEvent>
  ): Array<TypedListener<TEvent>>;
  listeners(eventName?: string): Array<Listener>;
  removeAllListeners<TEvent extends TypedEvent>(
    eventFilter: TypedEventFilter<TEvent>
  ): this;
  removeAllListeners(eventName?: string): this;
  off: OnEvent<this>;
  on: OnEvent<this>;
  once: OnEvent<this>;
  removeListener: OnEvent<this>;

  functions: {
    addBook(
      _isbn: PromiseOrValue<string>,
      _title: PromiseOrValue<string>,
      _author: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<ContractTransaction>;

    books(
      arg0: PromiseOrValue<BigNumberish>,
      overrides?: CallOverrides
    ): Promise<
      [BigNumber, string, string, string, boolean, string] & {
        id: BigNumber;
        isbn: string;
        title: string;
        author: string;
        borrowed: boolean;
        borrowingAddress: string;
      }
    >;

    borrowBook(
      _title: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<ContractTransaction>;

    copies(
      arg0: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<[number]>;

    getBooks(
      overrides?: CallOverrides
    ): Promise<[LibraryFactory.BookStructOutput[]]>;

    historicalBorrowers(
      arg0: PromiseOrValue<string>,
      arg1: PromiseOrValue<BigNumberish>,
      overrides?: CallOverrides
    ): Promise<[string]>;

    owner(overrides?: CallOverrides): Promise<[string]>;

    renounceOwnership(
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<ContractTransaction>;

    returnBook(
      _title: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<ContractTransaction>;

    showAvailableBooks(overrides?: CallOverrides): Promise<[string[]]>;

    titleToIsbn(
      arg0: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<[string]>;

    transferOwnership(
      newOwner: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<ContractTransaction>;
  };

  addBook(
    _isbn: PromiseOrValue<string>,
    _title: PromiseOrValue<string>,
    _author: PromiseOrValue<string>,
    overrides?: Overrides & { from?: PromiseOrValue<string> }
  ): Promise<ContractTransaction>;

  books(
    arg0: PromiseOrValue<BigNumberish>,
    overrides?: CallOverrides
  ): Promise<
    [BigNumber, string, string, string, boolean, string] & {
      id: BigNumber;
      isbn: string;
      title: string;
      author: string;
      borrowed: boolean;
      borrowingAddress: string;
    }
  >;

  borrowBook(
    _title: PromiseOrValue<string>,
    overrides?: Overrides & { from?: PromiseOrValue<string> }
  ): Promise<ContractTransaction>;

  copies(
    arg0: PromiseOrValue<string>,
    overrides?: CallOverrides
  ): Promise<number>;

  getBooks(
    overrides?: CallOverrides
  ): Promise<LibraryFactory.BookStructOutput[]>;

  historicalBorrowers(
    arg0: PromiseOrValue<string>,
    arg1: PromiseOrValue<BigNumberish>,
    overrides?: CallOverrides
  ): Promise<string>;

  owner(overrides?: CallOverrides): Promise<string>;

  renounceOwnership(
    overrides?: Overrides & { from?: PromiseOrValue<string> }
  ): Promise<ContractTransaction>;

  returnBook(
    _title: PromiseOrValue<string>,
    overrides?: Overrides & { from?: PromiseOrValue<string> }
  ): Promise<ContractTransaction>;

  showAvailableBooks(overrides?: CallOverrides): Promise<string[]>;

  titleToIsbn(
    arg0: PromiseOrValue<string>,
    overrides?: CallOverrides
  ): Promise<string>;

  transferOwnership(
    newOwner: PromiseOrValue<string>,
    overrides?: Overrides & { from?: PromiseOrValue<string> }
  ): Promise<ContractTransaction>;

  callStatic: {
    addBook(
      _isbn: PromiseOrValue<string>,
      _title: PromiseOrValue<string>,
      _author: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<void>;

    books(
      arg0: PromiseOrValue<BigNumberish>,
      overrides?: CallOverrides
    ): Promise<
      [BigNumber, string, string, string, boolean, string] & {
        id: BigNumber;
        isbn: string;
        title: string;
        author: string;
        borrowed: boolean;
        borrowingAddress: string;
      }
    >;

    borrowBook(
      _title: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<void>;

    copies(
      arg0: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<number>;

    getBooks(
      overrides?: CallOverrides
    ): Promise<LibraryFactory.BookStructOutput[]>;

    historicalBorrowers(
      arg0: PromiseOrValue<string>,
      arg1: PromiseOrValue<BigNumberish>,
      overrides?: CallOverrides
    ): Promise<string>;

    owner(overrides?: CallOverrides): Promise<string>;

    renounceOwnership(overrides?: CallOverrides): Promise<void>;

    returnBook(
      _title: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<void>;

    showAvailableBooks(overrides?: CallOverrides): Promise<string[]>;

    titleToIsbn(
      arg0: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<string>;

    transferOwnership(
      newOwner: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<void>;
  };

  filters: {
    "OwnershipTransferred(address,address)"(
      previousOwner?: PromiseOrValue<string> | null,
      newOwner?: PromiseOrValue<string> | null
    ): OwnershipTransferredEventFilter;
    OwnershipTransferred(
      previousOwner?: PromiseOrValue<string> | null,
      newOwner?: PromiseOrValue<string> | null
    ): OwnershipTransferredEventFilter;
  };

  estimateGas: {
    addBook(
      _isbn: PromiseOrValue<string>,
      _title: PromiseOrValue<string>,
      _author: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<BigNumber>;

    books(
      arg0: PromiseOrValue<BigNumberish>,
      overrides?: CallOverrides
    ): Promise<BigNumber>;

    borrowBook(
      _title: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<BigNumber>;

    copies(
      arg0: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<BigNumber>;

    getBooks(overrides?: CallOverrides): Promise<BigNumber>;

    historicalBorrowers(
      arg0: PromiseOrValue<string>,
      arg1: PromiseOrValue<BigNumberish>,
      overrides?: CallOverrides
    ): Promise<BigNumber>;

    owner(overrides?: CallOverrides): Promise<BigNumber>;

    renounceOwnership(
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<BigNumber>;

    returnBook(
      _title: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<BigNumber>;

    showAvailableBooks(overrides?: CallOverrides): Promise<BigNumber>;

    titleToIsbn(
      arg0: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<BigNumber>;

    transferOwnership(
      newOwner: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<BigNumber>;
  };

  populateTransaction: {
    addBook(
      _isbn: PromiseOrValue<string>,
      _title: PromiseOrValue<string>,
      _author: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<PopulatedTransaction>;

    books(
      arg0: PromiseOrValue<BigNumberish>,
      overrides?: CallOverrides
    ): Promise<PopulatedTransaction>;

    borrowBook(
      _title: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<PopulatedTransaction>;

    copies(
      arg0: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<PopulatedTransaction>;

    getBooks(overrides?: CallOverrides): Promise<PopulatedTransaction>;

    historicalBorrowers(
      arg0: PromiseOrValue<string>,
      arg1: PromiseOrValue<BigNumberish>,
      overrides?: CallOverrides
    ): Promise<PopulatedTransaction>;

    owner(overrides?: CallOverrides): Promise<PopulatedTransaction>;

    renounceOwnership(
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<PopulatedTransaction>;

    returnBook(
      _title: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<PopulatedTransaction>;

    showAvailableBooks(
      overrides?: CallOverrides
    ): Promise<PopulatedTransaction>;

    titleToIsbn(
      arg0: PromiseOrValue<string>,
      overrides?: CallOverrides
    ): Promise<PopulatedTransaction>;

    transferOwnership(
      newOwner: PromiseOrValue<string>,
      overrides?: Overrides & { from?: PromiseOrValue<string> }
    ): Promise<PopulatedTransaction>;
  };
}
